...
export default function SearchPage() {
  ...
  // เมื่อมีการส่ง query parameter มาจากหน้าอื่น
  useEffect(() => {
    if (router.query.q) {
  ...
  useEffect(() => {
    const fetchTemples = async () => {
      try {
        // ข้อมูลวัดทั้งหมด (รวมเพิ่มเติมจากหน้า sacred-places)
        setTemples([
          {
            id: 1,
  ...
  const handleTempleClick = (templeId: number) => {
    if (templeId === 1) {
      ...
      // สำหรับวัดอื่นๆ สามารถเพิ่ม logic ตามต้องการ
      console.log(`Clicked temple ID: ${templeId}`);
    }
  };
  ...
  return (
    <>
      <Head>
        <title>ค้นหาสถานที่ศักดิ์สิทธิ์ - Nummu App</title>
        <meta
          name="description"
      ...
      <div className="min-h-screen bg-gray-100 pb-20">
        ...
        <div className="bg-orange-500 text-white p-4 rounded-b-3xl">
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="ค้นหาชื่อวัด สถานที่ หรือที่อยู่..."
              className="w-full bg-white rounded-full px-10 py-3 text-gray-700 text-base"
              value={searchQuery}
          ...
          {searchQuery && (
            <div className="mb-4">
              <p className="text-gray-600 text-sm">
                พบ {filteredTemples.length} รายการจากการค้นหา "{searchQuery}"
              </p>
            </div>
          )}
