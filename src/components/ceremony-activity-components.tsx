      ...
      {items.map(item => (
        ...
        >
          ...
          <div 
            ...
            onClick={() => onItemClick(item.id)}
          >
            <h3 className="text-orange-600 font-medium text-center">{item.name}</h3>
            <p className="text-gray-600 text-xs text-center mt-1">{item.description}</p>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className={`min-h-screen ${backgroundClass}`}>
      ...
      <CeremonyActivityHeader 
        onBackClick={handleBackClick}  // ส่งต่อไปให้ Header
        title={type === 'ceremony' ? 'พิธีกรรม' : 'กิจกรรม'}
        userName={userName}
      />
